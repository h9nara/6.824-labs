...
func (cfg *config) checkLogs(i int, m ApplyMsg) (string, bool) {
	...
	for j := 0; j < len(cfg.logs); j++ {
		if old, oldok := cfg.logs[j][m.CommandIndex]; oldok && old != v {
			log.Printf("S%d: log %v; S%d %v\n", i, cfg.logs[i], j, cfg.logs[j])
			// some server has already committed a different value for this entry!
			err_msg = fmt.Sprintf("commit index=%v S%d %v != S%d %v",
...
func (cfg *config) one(cmd interface{}, expectedServers int, retry bool) int {
	...
	for time.Since(t0).Seconds() < 20 && cfg.checkFinished() == false {
		...
		if index != -1 {
			...
			if retry == false {
				cfg.t.Fatalf("one(%v) failed to reach agreement, expect %d servers", cmd, expectedServers)
			}
		} else {
	...
	if cfg.checkFinished() == false {
		cfg.t.Fatalf("one(%v) failed to reach agreement with timeout, expect %d servers", cmd, expectedServers)
	}
	return -1
}
